...
Suffer::SystemBlueRender::SystemBlueRender(){
...
void Suffer::SystemBlueRender::Execute(GameObject * go){
  ...
  if (!go->HasComponent(Component::ComponentKind::kComponentKind_BlueRender)) return;
  ...
  if (!go->HasComponent(Component::kComponentKind_Material)) {
    ...
    material_params->color_ = mathmorra::Vector4(0.0f, 0.0f, 1.0f, 1.0f);
